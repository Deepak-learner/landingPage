...
function validate() {
    ...
    if (result == "") {
        alert(
            " Your details has stored successfully. We will contact you shortly."
        );
        return true;
    }
...
function validatePhoneNumber() {
    ...
    if (reg.test(password)) {
        document.getElementById("invalidPhoneNumber").innerHTML =
            "Error: Phone number should only contain numeric characters.\n";
        return "error";
    } else if (password.length < 10 || password.length > 10) {
        document.getElementById("invalidPhoneNumber").innerHTML =
            "Error: Phone number should be 10 charater long.\n";
        return "error";
    }
    return "";
}
